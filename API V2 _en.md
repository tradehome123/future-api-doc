...
BW-TIMESTAMP: Time the request was initiated（UTC），example：2021-01-05T14:05:28.616Z
...
- BW-APIKEY: api key
- BW-TIMESTAMP: Request time, in ISO format，example`2021-01-05T14:05:28.616Z
- BW-SIGN：signature
- BW-LAN: language，support cn(chinese)、en(english) and kr(korean)，the default is cn



### 1.3. Signature rules

**This platform provides python, java, go versions of the api signature request demo, see:**<br>
python version: https://github.com/tradehome123/future-api-python <br>
java version: https://github.com/tradehome123/future-api-java <br>
Go version: https://github.com/tradehome123/future-api-go <br>

**Another code example of the signature part in chapters 1.3.6 and 1.3.7 can be referred to**



#### 1.3.1. ApiKey

The user's api key is generated by the bw platform
...
- The request header of BW-SIGN is obtained by encrypting``timestamp`` + ``method``  + ``requestPath`` + ``request query/request body the string`` (+ means string connection) and SecretKey using the HMAC SHA256 method and outputting it through Base64 encoding.；

  example：`sign=CryptoJS.enc.Base64.Stringify(CryptoJS.HmacSHA256(timestamp + 'GET' + '/users/self/verify', SecretKey))`

  Among them, the value of `timestamp`is the same as the request header of`BW-TIMESTAMP`in ISO format, such as`2021-01-05T14:05:28.616Z`.
...
For example：Request header BW-TIMESTAMP: 2021-01-05T14:05:28.616Z，method: GET，Request interface path：/Server/api/v1/trade/getOrder，Then the final string for signature calculation is
...
- URL: /Server/api/v2/Fund/transferFund
  ...
  - Request Parameter: 
    ...
    |side |Yes  |Integer | 1：Deposit (bw account -> futures account)，0：Withdrawal (futures account -> bw account)    |
...
After the connection is established, you need to log in before you can subscribe to the channel. When subscribing to future channels, you do not need to bring BW-APIKEY, BW-TIMESTAMP and BW-SIGN.
...
- **Request parameter：：**
  ...
  | BW-APIKEY    | yes  | String | The user's api key is generated by the BW platform                        |
  | BW-TIMESTAMP | yes   | String | Request time, in ISO format, such as`2021-01-05T14:05:28.616Z |
  | BW-SIGN      | yes   | String | sign                                             |
...
- BW-SIGN field is obtained by encrypting``timestamp`` + ``GET``  + ``login``(+means string connection))，and SecretKey，using the HMAC SHA256 method and outputting through Base64 encoding;

  For example：`sign=CryptoJS.enc.Base64.Stringify(CryptoJS.HmacSHA256(timestamp + 'GET' + 'login', SecretKey))`

  Among them，the value of `timestamp` is same as the request of `BW-TIMESTAMP` in ISO format,such as `2021-01-05T14:05:28.616Z`。
